...
def make_env_atari(gym_id, seed, idx, capture_video, run_name):
    def thunk():
        ...
        if capture_video and idx == 0:
            env = gym.wrappers.RecordVideo(
                env, f"videos/{run_name}", step_trigger=lambda t: t % 2 == 0
            )
...
def make_env_continuous(gym_id, seed, idx, capture_video, run_name):
    def thunk():
        ...
        env = gym.wrappers.RecordEpisodeStatistics(env)
        if capture_video and idx == 0:
            print("Record video: True")
            env = gym.wrappers.RecordVideo(
                env, f"videos/{run_name}", render_mode="human"
            )
        ### Detail 4: Clip the actions to it's lower and uper bound
        #   Note: The out of bounds actions are stored for the forward pass
        #   But the env executes the clipped action
        env = gym.wrappers.ClipAction(env)
        ### Detail 5: Normalize (standardize) the observations
        env = gym.wrappers.NormalizeObservation(env)
        ### Detail 6: Clip the observations to be in [-10, 10]
        env = gym.wrappers.TransformObservation(env, lambda obs: np.clip(obs, -10, 10))
        ### Detail 7: Normalize the rewards
        #   Note: The implementation normalizes the ```return``` instead of the reward
        env = gym.wrappers.NormalizeReward(env)
        ### Detail 8: Clip the rewards to be in [-10, 10]
...
def make_envs_continuous(gym_id, seed, num_envs, capture_video, run_name):
    return gym.vector.SyncVectorEnv(
        [
            make_env_continuous(gym_id, seed + i, i, capture_video, run_name)
...
def make_deploy_env_atari(gym_id):
